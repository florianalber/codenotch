...
enum UsageBand: String, Codable, Equatable {
    ...
    /// The band a window shows, with what its own forecast says folded in.
    ///
    /// A window on course to be spent before it resets is worth watching even
    /// at 20%, which is the one thing a percentage cannot say. It can only ever
    /// raise the band, never lower it — a forecast must not make a nearly-spent
    /// limit look better than it is, and "you will run out" is not news beside
    /// "you nearly have".
    static func band(for usedFraction: Double, watchLimit: Double = 0.50, criticalLimit: Double = 0.70,
                     runsOutBeforeReset: Bool) -> UsageBand {
        let plain = band(for: usedFraction, watchLimit: watchLimit, criticalLimit: criticalLimit)
